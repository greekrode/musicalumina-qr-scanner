...
import { Camera, X, Copy, History, Music } from 'lucide-react';
import { isJWT, decodeJWT, DecodedJWT } from '../utils/jwtDecoder';
      ...
      <div className="p-4 space-y-6">
              ...
              {!hasCamera && (
                <div className="absolute inset-0 bg-musica-burgundy/10 rounded-xl flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-musica-burgundy/60">Camera not available</p>
                  </div>
                </div>
              )}
            </div>
        ...
        {scanResult && !decodedJWT && (
          <div className="bg-emerald-50/80 backdrop-blur-lg border border-emerald-200 rounded-2xl p-6 shadow-lg">
            ...
            <div className="bg-white/60 rounded-lg p-4 mb-4 border border-emerald-100">
              <p className="text-musica-burgundy font-mono text-sm break-all">{scanResult}</p>
            </div>
        ...
        {showHistory && (
          <div className="bg-white/80 backdrop-blur-lg border border-musica-burgundy/10 rounded-2xl p-6 shadow-xl">
            ...
            {scanHistory.length === 0 ? (
              <p className="text-musica-burgundy/60 text-center py-8">No scans yet</p>
            ) : (
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {scanHistory.map((scan) => (
                  <div key={scan.id} className="bg-musica-cream/50 rounded-lg p-4 border border-musica-burgundy/5">
